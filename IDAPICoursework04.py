...
def Covariance(theData):
    ...
    # covar = cov(array(realData).T)
    covar = cov(realData, rowvar=0)
...
def PrincipalComponents(theData):
    ...
    UUT = dot(U, U.T)
    L, phiPrime = linalg.eig(UUT)

    #4. Compute the eigenvector of the covariance matrix S
    UTPhiPrime = numpy.dot(U.T, phiPrime)    

    #5. Orthonormalise by dividing each column of UT·Φ' by its magnitude sqrt(λi)
    for i in range(len(L)):
        UTPhiPrime[:, i] /= linalg.norm(UTPhiPrime[:, i])

    #6. Sort the Principal Components by the largest eigenValue
    sorting = argsort(L)
    for i in reversed(sorting):
        orthoPhi.append(UTPhiPrime[:, i])
...
CreatePartialReconstructions(pc, Mean(faces), cFaceProjPC)
