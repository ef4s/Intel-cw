...
def ProjectFace(theBasis, theMean, theFaceImage):
    ...
    uuT = dot(theBasis.T, theBasis)
    
#    theMean = numpy.mean(theFaceImage)
    xBar = theFaceImage - theMean
    
    magnitudes = dot(uuT, xBar)
...
def CreatePartialReconstructions(aBasis, aMean, componentMags):
    a = aBasis * (componentMags - aMean)
    a += aMean
    
    return a    

def CreateReconstructions(theBasis, theFace):
    tFace = list(theFace)
    
    for i in range(len(theBasis)):
        m = numpy.mean(tFace)
        tFace = CreatePartialReconstructions(theBasis[i], m, tFace)
        SaveEigenface(tFace,"meanFace" + str(i) + ".jpg")
...
CreateReconstructions(eigenFaceBasis, face)
