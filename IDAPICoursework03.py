...
def BestNetwork(theData, arcList, cptList, noStates):
    ...
    for i in range(len(arcList)):        
        ...
        if i > 0:
            arcListPrime += arcList[:i]
            cptListPrime += cptList[:i]

        if i < len(arcList) - 1: # Note, the -1 is due to the 0 based indexing
            arcListPrime += arcList[i + 1:]
            cptListPrime += cptList[i + 1:]
